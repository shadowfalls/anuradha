...
export default class Editable extends React.Component {
  ...
  componentDidMount() {
    if (!this.contentEditable || !this.contentEditable.current) return;
    if (this.props.isAutoFocus && this.props.index === 0) this.contentEditable.current.focus();
    ...
    if (!this.contentEditable.current.onkeyup) {
      this.contentEditable.current.onkeyup = (event) => {
        ...
        // Down arrow press
        if (event.keyCode === 40 && (event.shiftKey || event.ctrlKey)) this.props.setFocusToIndex(this.props.index + 1);
        // Up arrow press
        if (event.keyCode === 38 && this.props.index - 1 >= 0 && (event.shiftKey || event.ctrlKey)) this.props.setFocusToIndex(this.props.index - 1);
      };
    }
    ...
    if (!this.contentEditable.current.onkeydown) {
      this.contentEditable.current.onkeydown = (event) => {
        switch (event.keyCode) {
          ...
          case 8:
            if (!this.props.data.html || this.props.data.html === '<br>') this.props.removeItem(this.props.index);
            break;
          default:
            break;
        }
    ...
    if (!this.contentEditable.current.onfocus) {
      this.contentEditable.current.onfocus = () => {
        if (this.props.isLast) this.props.setFocusToEnd();
        else this.props.setCurrentIndex(this.props.index);
      };
    }
  ...
  render() {
    ...
    const classList = [];
    if (!this.props.isLast) classList.push('mt-3');
    if (this.props.data.isQuoted) classList.push('quote');
    if (this.props.data.isMainHeading) classList.push('main-heading');
    if (this.props.data.isSubHeading) classList.push('sub-heading');
    if (this.props.data.isCodeSection) classList.push('code-section');
